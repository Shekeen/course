...
instance Show Digit3 where
  show d = let showd x = toLower `fmap` show x
...
toDot :: String -> ([Digit], String)
toDot = let toDot' x [] = (x, [])
            toDot' x (h:t) = let move = case fromChar h of
                                            Just n -> toDot' . (:) n
                                            Nothing -> if h == '.'
                                                       then (,)
                                                       else toDot'
...
illionate :: [Digit] -> String
illionate = let space "" = ""
                space x = ' ' : x
                todigits acc _ [] = acc
                todigits _ [] _ = error "unsupported illion"
                todigits acc (_:is) (Zero:Zero:Zero:t) = todigits acc is t
                todigits acc (i:is) (q:r:s:t) = todigits ((show (D3 s r q) ++ space i) : acc) is t
                todigits acc (_:is) (Zero:Zero:t) = todigits acc is t
                todigits acc (i:_) (r:s:_) = (show (D2 s r) ++ space i) : acc
                todigits acc (_:is) (Zero:t) = todigits acc is t
                todigits acc (i:_) (s:_) = (show (D1 s) ++ space i) : acc
...
dollars x = let (d, c) = toDot (dropWhile (`notElem` ('.':['1'..'9'])) x)
                c' = case mapMaybe fromChar c of
                       [] -> "zero cents"
                       [Zero, One] -> "one cent"
